...
def main():
    ...
    parser = binhoArgumentParser(
        description="Simple UART monitor.", verbose_by_default=True
    )
    parser.add_argument(
        "baud",
        nargs="?",
    ...
    parser.add_argument(
        "-B", "--bits", type=int, default=8, help="The number of data bits per frame."
    )
    parser.add_argument(
        "-S", "--stop", type=int, default=1, help="The number of stop bits per frame."
    )
    parser.add_argument(
        "-P",
        "--parity",
    ...
    input_thread = threading.Thread(
        target=input_handler, args=(console, input_queue, termination_request)
    )
    input_thread.start()

    # Configure our UART parameters.
    device.uart.update_parameters(
        baud=args.baud,
        data_bits=args.data,
        stop_bits=args.stop,
        parity=parity_modes[args.parity],
    )
    ...
    while True:
        ...
        # If we're preforming newline translation, prepend a "\r" to any
        # newline.
